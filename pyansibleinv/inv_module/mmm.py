...
Usage:
  pyansibleinv mmm [--osver OSVER] [--monitor_vip MONVIP] [--password PASSWORD] [--workdir WORKDIR] [--sshpass SSHPASS] [--sshport SSHPORT] [--sshkey SSHKEY] [--hostarg HOSTARG] [--ssh_try_limit SSHLIMIT] [--taskid TASKID] [--without_backup] --cluster_id CLUSTERID --data_host DATAHOSTS --monitor_host MONHOSTS --writer_vip WVIP --reader_vip RVIP
...
def gen_inv(args):
    ...
    for i, host_info in enumerate(mmm_dict['monitor_hosts']):
        ...
        hosts_script.append('{:<60}{:<60}ansible_ssh_port={:<7}{} {}'.format(k, 'ansible_ssh_host='+v, str(mmm_dict['sshport']), ansible_auth, mmm_dict['hostarg']))
