...
def gen_inv(args):
    ...
    hosts_script.append('{:<60} ansible_ssh_host={:<30} ansible_ssh_user={:<20} ansible_ssh_pass={:<20} ansible_ssh_port={:<10}  ansible_become_pass={:<20} ansible_winrm_transport=ntlm ansible_connection=winrm ansible_winrm_server_cert_validation=ignore'.format(mssql_dict['hostname'], +mssql_dict['ip'], mssql_dict['sshuser'], mssql_dict['sshpass'], mssql_dict['sshport'], mssql_dict['sshpass']))
